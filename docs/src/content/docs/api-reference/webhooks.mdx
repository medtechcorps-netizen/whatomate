...
ReReply publishes the exact callback in **Settings → Integrations → WhatsApp & Meta**. Use that value rather than constructing a callback manually. A shared platform app uses `/api/webhook`; a workspace-owned app includes a public tenant selector such as `/api/webhook?workspace={organization_id}`.

### Verification Endpoint

```bash
GET /api/webhook[?workspace={organization_id}]
```
...
| `hub.verify_token` | The write-only token configured in Integration Center (or the deployment-managed platform token) |
| `hub.challenge` | Challenge string to return |

### Event Endpoint

```bash
POST /api/webhook[?workspace={organization_id}]
```
