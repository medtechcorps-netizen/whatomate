...
s3_key = ""
s3_secret = ""
s3_endpoint = ""             # S3-compatible endpoint; leave empty for AWS S3
s3_use_path_style = false    # Set only when the provider requires path-style URLs
...
<Aside type="note">
  Workspace Meta app credentials and AI provider keys are configured **per organization** under **Settings → Integrations**. WhatsApp account identifiers and access tokens are configured under **Settings → Accounts**.
</Aside>
...
- `type = "local"` — files under `local_path` (default `./uploads`). Suitable for local development and single-host deployments.
- `type = "s3"` — stores new tenant message and campaign media in S3-compatible object storage. Fill in `s3_bucket`, `s3_region`, `s3_key`, and `s3_secret`. Set `s3_endpoint` for Railway Buckets, Cloudflare R2, or another compatible provider. `s3_use_path_style` is only needed when the provider credentials explicitly require path-style URLs.

S3 object keys are prefixed with `organizations/<organization-id>/`, and media is served through authenticated API routes. Existing local media remains readable during migration.

For a Railway Bucket, map the generated credentials as follows:

| Railway credential | Whatomate field |
|---|---|
...
| `urlStyle = "virtual-host"` | `s3_use_path_style = false` |
| `urlStyle = "path"` | `s3_use_path_style = true` |

Use the `region` value returned with the credentials (often `auto`), which can differ from the physical bucket location selected in Railway.
...
| `WHATOMATE_STORAGE__S3_ENDPOINT` | `storage.s3_endpoint` |
...
For a workspace-owned Meta app, configure application credentials once per organization:

1. Navigate to **Settings → Integrations → WhatsApp & Meta** as a workspace administrator.
2. Enter the **Meta App ID**, **Embedded Signup Config ID**, write-only **App Secret**, and write-only **Webhook Verify Token**.
3. Copy the exact webhook callback shown by Integration Center into Meta for Developers.
4. Navigate to **Settings → Accounts**, click **Add Account**, and enter only the account-owned values:
...
A deployment using the shared platform Meta app shows the platform app and webhook token as read-only. Workspace administrators still add each account's Phone Number ID, Business Account ID, and access token.

<Aside type="caution">
  App secrets, webhook verify tokens, and account access tokens are write-only and encrypted at rest using `app.encryption_key`. ReReply rejects new credential writes when server-side encryption is unavailable.
</Aside>
