...
import type {
  PanelConfig,
  AvailableVariable,
} from "@/components/chatbot/PanelConfigEditor.vue";
...
function toGraphPayload(): ChatFlowGraph {
  ...
  if (start) {
    ...
    entry =
      ivrNodes.find((n) => !nodesWithIncoming.has(n.id))?.id ||
      ivrNodes[0]?.id ||
      "";
  }
...
function firstSupportedNodeValidationIssue(
  graph: ChatFlowGraph,
...
const availableVariables = computed<AvailableVariable[]>(() => {
  ...
  for (const n of nodes.value) {
    ...
    if (
      n.type === "prompt" &&
      typeof cfg.store_as === "string" &&
      cfg.store_as.trim()
    ) {
      out.push({
        key: cfg.store_as.trim(),
        source: "Store as",
        stepName: n.id,
      });
    }
    if (
      ...
      typeof cfg.response_mapping === "object"
    ) {
    ...
    if (
      n.type === "set_variable" &&
      cfg.set &&
      typeof cfg.set === "object" &&
      !Array.isArray(cfg.set)
    ) {
      for (const key of Object.keys(cfg.set)) {
        if (key.trim())
          out.push({ key: key.trim(), source: "Set variable", stepName: n.id });
      }
    }
  }
  return out;
...
async function loadFlow() {
  ...
  try {
    ...
    completionMessage.value =
      flow.completion_message || flow.CompletionMessage || "";
    onCompleteAction.value = (flow.on_complete_action ||
      flow.OnCompleteAction ||
      "none") as "none" | "webhook";
    ...
    createdByName.value =
      flow.created_by_name || flow.created_by?.full_name || "";
    updatedByName.value =
      flow.updated_by_name || flow.updated_by?.full_name || "";
...
async function loadAvailableFlows() {
  try {
    ...
    const list =
      (res.data as any)?.data?.flows || (res.data as any)?.flows || [];
    availableFlows.value = list.map((f: any) => ({
      id: f.id || f.ID,
      name: f.name || f.Name,
    }));
  } catch {
...
async function saveFlow() {
  ...
  try {
    const data: Record<string, any> = {
      ...
      trigger_keywords: triggerKeywords.value
        .split(",")
        .map((k) => k.trim())
        .filter(Boolean),
      ...
      completion_config:
        onCompleteAction.value === "webhook" ? completionConfig.value : {},
    ...
    if (isNewFlow.value) {
      ...
      toast.success(
        t("common.savedSuccess", { resource: t("resources.Flow") }),
      );
    }
...
function addCompletionHeader() {
  completionConfig.value.headers = {
    ...(completionConfig.value.headers || {}),
    "": "",
  };
  hasUnsavedChanges.value = true;
...
<template>
  <div class="flex flex-col h-screen bg-muted/30">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <Button @click="saveFlow" :disabled="isSaving">
            ...
            {{
              isSaving
                ? $t("flowBuilder.saving") + "..."
                : $t("flowBuilder.saveFlow")
            }}
          </Button>
        </div>
      </div>
    ...
    <!-- Node palette -->
    <div
      class="flex items-center gap-2 px-4 py-2 border-b bg-muted/30 overflow-x-auto shrink-0"
    >
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="flex-1 relative">
        ...
        <ErrorState
          ...
          :description="
            $t(
              'flowBuilder.loadFailedDesc',
              'Try reloading or go back to the flow list.',
            )
          "
          ...
          <template #action>
            <div class="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                @click="router.push('/chatbot/flows')"
              >
      ...
      <!-- Right panel -->
      <Card
        class="w-[420px] min-w-0 border-y-0 border-r-0 rounded-none shrink-0 flex flex-col"
      >
        <!-- Node properties when a node is selected -->
        <div
          v-if="selectedChatNode && selectedChatNode.type !== 'start'"
          ...
          <div class="p-4 space-y-4 min-w-0">
            ...
            <div class="space-y-1.5">
              ...
              <p class="text-[10px] text-muted-foreground">
                {{ $t("flowBuilder.triggerKeywordsHint") }}
              </p>
            </div>
            ...
            <Collapsible v-model:open="completionConfigOpen">
              ...
              >
                ...
                <component
                  :is="completionConfigOpen ? ChevronDown : ChevronRight"
                  class="h-4 w-4"
                />
              </CollapsibleTrigger>
                  ...
                  <Select v-model="onCompleteAction">
                    ...
                    <SelectContent>
                      <SelectItem value="none">{{
                        $t("flowBuilder.noAction")
                      }}</SelectItem>
                      <SelectItem value="webhook">{{
                        $t("flowBuilder.sendToWebhook")
                      }}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <div class="space-y-3 p-3 border rounded-lg bg-muted/30">
                    <div class="flex gap-2">
                      ...
                      <div class="flex-1">
                        ...
                        <Input
                          v-model="completionConfig.url"
                          placeholder="https://example.com/hook"
                          class="h-7 text-xs font-mono"
                        />
                        <p class="mt-1 text-[9px] text-muted-foreground">
                          Use variable placeholders for query values; keep
                    ...
                    <div class="space-y-2">
                      <div class="flex items-center justify-between">
                        <Label class="text-[10px]">{{
                          $t("flowBuilder.headers")
                        }}</Label>
                        <Button
                          variant="ghost"
                          size="sm"
                          class="h-5 text-[10px] px-1"
                          @click="addCompletionHeader"
                        >
                          <Plus class="h-3 w-3" />
                        </Button>
                      </div>
                      <div
                        v-for="(val, key) in completionConfig.headers"
                        :key="key"
                        class="flex gap-1"
                      >
                        <Input
                          ...
                          @update:model-value="
                            (v: string) =>
                              updateCompletionHeaderKey(String(key), v)
                          "
                          placeholder="Key"
                        ...
                        <Input
                          ...
                          @update:model-value="
                            (v: string) =>
                              updateCompletionHeaderValue(String(key), v)
                          "
                          placeholder="Value"
                          class="h-6 text-[10px] flex-1"
                        />
                        <Button
                          ...
                          size="icon"
                          class="h-5 w-5"
                          @click="removeCompletionHeader(String(key))"
                        >
                    ...
                    <div class="space-y-1">
                      ...
                      <p class="text-[9px] text-muted-foreground">
                        Body text is not a secret vault; header values are
            ...
            <Collapsible v-model:open="panelConfigOpen">
              ...
              >
                ...
                <component
                  :is="panelConfigOpen ? ChevronDown : ChevronRight"
                  class="h-4 w-4"
                />
              </CollapsibleTrigger>
    ...
    <ConfirmDialog
      ...
      :description="
        $t(
          'flowBuilder.deleteNodeConfirmDesc',
