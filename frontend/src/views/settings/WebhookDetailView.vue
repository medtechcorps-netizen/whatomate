...
const breadcrumbs = computed(() => [
  ...
  {
    label: isNew.value
      ? t("webhooks.newWebhook", "New Webhook")
      : webhook.value?.name || form.value.name || "",
  },
]);
...
function toggleEvent(eventValue: string, checked: boolean | "indeterminate") {
  if (checked === true) {
    if (!form.value.events.includes(eventValue))
...
async function save() {
  ...
  try {
    const payload = {
      ...
      secret: clearSecret.value ? undefined : form.value.secret || undefined,
      clear_secret: !isNew.value && clearSecret.value,
    ...
    if (isNew.value) {
      ...
      hasChanges.value = false;
      toast.success(
        t("common.createdSuccess", { resource: t("resources.Webhook") }),
      );
      ...
      toast.success(
        t("common.updatedSuccess", { resource: t("resources.Webhook") }),
      );
      await loadWebhook();
    }
  } catch (e) {
    toast.error(
      getErrorMessage(
        e,
        t("common.failedSave", { resource: t("resources.webhook") }),
      ),
    );
  } finally {
...
async function deleteWebhook() {
  ...
  try {
    ...
    toast.success(
      t("common.deletedSuccess", { resource: t("resources.Webhook") }),
    );
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedDelete", { resource: t("resources.webhook") }),
      ),
    );
  }
      ...
      <Card>
        <CardHeader class="pb-3">
          <div class="flex items-center justify-between">
            ...
            <Badge
              v-if="!isNew"
              :variant="(webhook?.is_active ?? true) ? 'default' : 'secondary'"
            >
              {{
        ...
        <CardContent class="space-y-4">
          <div class="space-y-1.5">
            <Label for="webhook-name" class="text-xs"
              >{{ $t("webhooks.name") }}
          ...
          <div class="space-y-1.5">
            <Label for="webhook-url" class="text-xs"
            ...
            <p class="text-xs text-muted-foreground">
              Query parameters are not supported for event webhooks. URLs and
              payloads are ordinary configuration; put API keys and tokens in
              Headers so ReReply encrypts and masks them.
            </p>
          </div>
          <div class="space-y-1.5">
            <Label class="text-xs"
            ...
            <div class="grid grid-cols-1 gap-2 border rounded-lg p-3">
              ...
              >
                <Checkbox
                  ...
                  @update:checked="
                    (checked) => toggleEvent(event.value, checked)
                  "
                  :disabled="!canWrite"
                />
                <div class="grid gap-0.5">
                  <Label :for="event.value" class="cursor-pointer">{{
                    event.label
                  }}</Label>
                  <p class="text-xs text-muted-foreground">
                    {{ event.description }}
          ...
          <div class="space-y-1.5">
            <Label for="webhook-hmac-secret" class="text-xs">{{
              $t("webhooks.secret")
            }}</Label>
            <Input
              ...
              @update:model-value="clearSecret = false"
            ...
            <div
              v-if="!isNew && webhook?.has_secret && canWrite"
              class="flex items-center justify-between gap-3 rounded-md border border-border/60 px-3 py-2"
            >
              <span class="text-xs text-muted-foreground">
                {{
                  clearSecret
                    ? "The stored signing secret will be removed when you save."
                    : "A write-only signing secret is stored."
                }}
              </span>
              <Button
                type="button"
                ...
                @click="clearSecret = !clearSecret"
              >
                {{ clearSecret ? "Keep secret" : "Remove secret" }}
              </Button>
            </div>
          </div>
          <div class="space-y-1.5">
          ...
          <div class="flex items-center justify-between">
            <Label class="text-xs font-normal cursor-pointer">{{
              $t("common.active")
            }}</Label>
            <Switch
    ...
    <AlertDialog v-model:open="deleteDialogOpen">
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction @click="deleteWebhook">{{
            $t("common.delete")
          }}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
