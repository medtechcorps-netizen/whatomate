...
const currentProviderConfig = computed(
  () => providerConfigs[editingProvider.value],
);
...
async function fetchProviders() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedLoad", { resource: t("resources.ssoProviders") }),
      ),
    );
  } finally {
...
async function saveProvider() {
  ...
  const existing = providers.value.find(
    (p) => p.provider === editingProvider.value,
  );
  ...
  if (editingProvider.value === "custom") {
    if (
      !editForm.value.auth_url ||
      !editForm.value.token_url ||
      !editForm.value.user_info_url
    ) {
  ...
  try {
    ...
    toast.success(
      t("common.savedSuccess", { resource: t("resources.SSOProvider") }),
    );
  } catch (error) {
    toast.error(
      getErrorMessage(
        error,
        t("common.failedSave", { resource: t("resources.SSOProvider") }),
      ),
    );
  } finally {
...
async function confirmDeleteProvider() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        error,
        t("common.failedDelete", { resource: t("resources.SSOProvider") }),
      ),
    );
  } finally {
...
<template>
  <div class="flex flex-col h-full bg-[#0a0a0b] light:bg-gray-50">
    ...
    <ScrollArea v-else class="flex-1">
      <div class="p-6">
        <div class="max-w-6xl mx-auto space-y-6">
          <!-- Info Card -->
          <Card
            class="bg-blue-950/30 light:bg-blue-50 border-blue-800 light:border-blue-200"
          >
            <CardContent class="flex items-start gap-3 pt-6">
              <Info
                class="h-5 w-5 text-blue-400 light:text-blue-600 shrink-0 mt-0.5"
              />
          ...
          <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card
              ...
              :class="
                ...
                (getConfiguredProvider(key)?.is_enabled
                  ? ' ring-2 ring-primary'
                  : '')
              "
            >
                ...
                >
                  <p>
                    <span class="font-medium"
                      >{{ $t("sso.autoCreateUsers") }}:</span
                    >
                    {{
                      getConfiguredProvider(key)?.allow_auto_create
                        ? $t("sso.yes")
                        : $t("sso.no")
                    }}
                  </p>
      ...
      <DialogContent class="max-w-lg">
        ...
        <div class="space-y-4 py-4 max-h-[60vh] overflow-y-auto">
          ...
          <div class="space-y-2">
            <Label for="client_secret">
              {{ $t("sso.clientSecret") }}
              <span
                v-if="getConfiguredProvider(editingProvider)?.has_secret"
        ...
        <DialogFooter class="flex gap-2">
          <Button
            ...
            size="sm"
            @click="
              openDeleteDialog(editingProvider);
    ...
    <ConfirmDialog
      ...
      :description="
        $t('sso.confirmDeleteDescription', {
          provider: providerConfigs[providerToDelete]?.name || '',
        })
      "
