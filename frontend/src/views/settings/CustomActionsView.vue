...
const columns = computed<Column<CustomAction>[]>(() => [
  ...
  {
    key: "status",
    label: t("customActions.status"),
    sortable: true,
    sortKey: "is_active",
  },
  {
    key: "created",
    label: t("customActions.created"),
    sortable: true,
    sortKey: "created_at",
  },
...
const getIconComponent = (iconName: string) =>
  iconOptions.find((i) => i.value === iconName)?.icon || Zap;
...
async function fetchActions() {
  ...
  try {
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedLoad", { resource: t("resources.customActions") }),
      ),
    );
  } finally {
...
async function saveAction() {
  if (!formData.value.name.trim()) {
    toast.error(t("customActions.nameRequired"));
    return;
  }
  if (
    (formData.value.action_type === "webhook" ||
      formData.value.action_type === "url") &&
    !formData.value.config.url.trim()
  ) {
    toast.error(t("customActions.urlRequired"));
    return;
  }
  if (
    formData.value.action_type === "javascript" &&
    !formData.value.config.code.trim()
  ) {
    toast.error(t("customActions.jsCodeRequired"));
    return;
  }
  ...
  switch (formData.value.action_type) {
    ...
    case "url":
      ...
      break;
    case "javascript":
      config = { code: formData.value.config.code };
      break;
  }
  ...
  try {
    ...
    if (isEditing.value && editingActionId.value) {
      await customActionsService.update(editingActionId.value, payload);
      toast.success(
        t("common.updatedSuccess", { resource: t("resources.CustomAction") }),
      );
    } else {
      await customActionsService.create(payload);
      toast.success(
        t("common.createdSuccess", { resource: t("resources.CustomAction") }),
      );
    }
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedSave", { resource: t("resources.customAction") }),
      ),
    );
  } finally {
...
async function performToggleAction(action: CustomAction) {
  ...
  try {
    ...
    toast.success(
      action.is_active
        ? t("common.disabledSuccess", { resource: t("resources.CustomAction") })
        : t("common.enabledSuccess", { resource: t("resources.CustomAction") }),
    );
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedUpdate", { resource: t("resources.customAction") }),
      ),
    );
  } finally {
...
async function deleteAction() {
  ...
  try {
    ...
    await fetchActions();
    toast.success(
      t("common.deletedSuccess", { resource: t("resources.CustomAction") }),
    );
    ...
    toast.error(
      getErrorMessage(
        e,
        t("common.failedDelete", { resource: t("resources.customAction") }),
      ),
    );
  } finally {
...
function addHeader() {
  if (newHeaderKey.value.trim() && newHeaderValue.value.trim()) {
    formData.value.config.headers[newHeaderKey.value.trim()] =
      newHeaderValue.value.trim();
    newHeaderKey.value = "";
...
function getActionTypeBadge(type: string) {
  return (
    {
      webhook: { label: "Webhook", variant: "default" as const },
      url: { label: "URL", variant: "secondary" as const },
      javascript: { label: "JavaScript", variant: "outline" as const },
          ...
          <Card>
            <CardHeader>
              <div class="flex items-center justify-between flex-wrap gap-4">
                <div>
                  ...
                  <CardDescription>{{
                    $t("customActions.yourActionsDesc")
                  }}</CardDescription>
                </div>
                <SearchInput
                  v-model="searchQuery"
                  :placeholder="$t('customActions.searchActions') + '...'"
                  class="w-64"
                />
              </div>
            </CardHeader>
            <CardContent>
              <DataTable
                ...
                :empty-title="
                  searchQuery
                    ? $t('customActions.noMatchingActions')
                    : $t('customActions.noActionsYet')
                "
                :empty-description="
                  searchQuery
                    ? $t('customActions.noMatchingActionsDesc')
                    : $t('customActions.noActionsYetDesc')
                "
              ...
              >
                <template #cell-icon="{ item: action }"
                  ><component
                    :is="getIconComponent(action.icon)"
                    class="h-5 w-5 text-muted-foreground"
                /></template>
                <template #cell-name="{ item: action }"
                  ><span class="font-medium">{{ action.name }}</span></template
                >
                <template #cell-type="{ item: action }"
                  ><Badge
                    :variant="getActionTypeBadge(action.action_type).variant"
                    >{{ getActionTypeBadge(action.action_type).label }}</Badge
                  ></template
                >
                <template #cell-target="{ item: action }"
                  ><span
                    class="max-w-[200px] truncate text-muted-foreground block"
                    >{{
                      action.action_type === "javascript"
                        ? $t("customActions.customScript")
                        : action.config.url
                    }}</span
                  ></template
                >
                <template #cell-status="{ item: action }">
                  <div class="flex items-center gap-2">
                    <Switch
                      ...
                      @update:checked="handleToggleAction(action)"
                ...
                <template #cell-created="{ item: action }"
    ...
    <Dialog v-model:open="isDialogOpen">
      <DialogContent class="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{{
            isEditing
              ? $t("customActions.editTitle")
              : $t("customActions.createTitle")
          }}</DialogTitle>
          <DialogDescription>{{
          ...
          <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-2">
              <Button
                ...
                class="h-10 w-10"
                :class="{
                  'ring-2 ring-primary': formData.icon === iconOpt.value,
                }"
            ...
            >
              ...
              <div class="flex items-center space-x-2">
                <RadioGroupItem value="javascript" id="type-javascript" /><Label
                  ...
                  class="flex items-center gap-2 cursor-pointer font-normal"
                  ><Code class="h-4 w-4" />{{
                    $t("customActions.javascriptType")
                  }}</Label
                >
              </div>
            </RadioGroup>
            ...
            <div class="border-t pt-4 space-y-4">
              <div class="space-y-2">
                ...
                <p class="text-xs text-muted-foreground">
                  Query values must be
                  <code>&#123;&#123;variable&#125;&#125;</code> placeholders.
                  Put API keys and tokens in Headers so ReReply encrypts and
                  masks them.
                </p>
              </div>
              <div class="space-y-2">
                ...
                <Select v-model="formData.config.method"
                  ...
                  ><SelectContent
                    ...
                    ><SelectItem value="PATCH">PATCH</SelectItem></SelectContent
                  ></Select
                >
              </div>
              <div class="space-y-2">
                ...
                <div class="space-y-2">
                  <div
                    v-for="(_, key) in formData.config.headers"
                    :key="key"
                  ...
                  >
                    ...
                    ><span
                      ...
                      aria-label="Stored header value"
                      >********</span
                    >
                    <Button
                      variant="ghost"
              ...
              <div class="space-y-2">
                <div class="flex items-center justify-between">
                  ...
                  ><Button
                    ...
                    >{{ $t("customActions.insertTemplate") }}</Button
                  >
                </div>
                <Textarea
                  id="body"
                ...
                />
                <p class="text-xs text-muted-foreground">
                  {{ $t("customActions.bodyVariables") }} Body text is not a
                  secret vault; keep credentials in Headers.
                </p>
              </div>
            </div>
          </template>
            ...
            <div class="border-t pt-4 space-y-4">
              <div class="space-y-2">
                ...
                ><Input
                  ...
                  type="text"
                  inputmode="url"
                  :placeholder="$t('customActions.urlPlaceholder')"
                />
                <p class="text-xs text-muted-foreground">
                  {{ $t("customActions.urlHint") }} Use an HTTPS URL or an
                  application path beginning with <code>/</code>; executable
                  schemes are blocked.
                </p>
              </div>
              <div class="flex items-center space-x-2">
                ...
                /><Label for="new-tab" class="cursor-pointer">{{
                  $t("customActions.openInNewTab")
                }}</Label>
              </div>
            </div>
          </template>
            ...
            <div class="border-t pt-4 space-y-4">
              <div class="space-y-2">
                ...
                <Textarea
                  ...
                  :placeholder="$t('customActions.jsCodePlaceholder')"
                  class="font-mono text-sm min-h-[200px]"
                />
                <p class="text-xs text-muted-foreground">
                  {{ $t("customActions.jsReturnHint") }}
                </p>
              </div>
            </div>
          </template>
