...
function updateCallbackHeaderKey(
  event: CallbackEvent,
  oldKey: string,
  newKey: string,
) {
...
function updateCallbackHeaderValue(
  event: CallbackEvent,
  key: string,
  value: string,
) {
  ...
  <div class="space-y-4 p-4">
      ...
      <Tabs :default-value="greetingTab">
        ...
        <TabsContent value="audio" class="mt-2">
          <div class="flex items-center gap-2">
            <div
              v-if="config.audio_file"
              class="flex items-center gap-1 flex-1 min-w-0 px-2 py-1 border rounded-md bg-muted/50"
            >
              <Button
                variant="ghost"
                size="icon"
          ...
          <Textarea
            ...
            @update:model-value="
              (v: string) =>
                updateConfigEntries(
                  v
                    ? { greeting_text: v, audio_file: undefined }
                    : { greeting_text: v },
                )
            "
    ...
    <div v-if="node.type === 'greeting'" class="flex items-center gap-2">
      <Switch
        ...
        @update:checked="(v: boolean) => updateConfig('interruptible', v)"
      />
    ...
    <template v-if="node.type === 'menu'">
      <div class="space-y-1.5">
        ...
        <Input
          type="number"
          :model-value="String(config.timeout_seconds || 10)"
          @update:model-value="
            (v: string) => updateConfig('timeout_seconds', parseInt(v) || 10)
          "
          class="h-8 text-sm"
          min="1"
          max="60"
      ...
      <div class="space-y-1.5">
        ...
        <div
          v-for="(opt, digit) in config.options || {}"
          :key="String(digit)"
        ...
        >
          ...
          <Input
            :model-value="(opt as any)?.label || ''"
            @update:model-value="
              (v: string) => updateMenuOption(String(digit), 'label', v)
            "
            placeholder="Label"
          ...
          <Button
            ...
            size="icon"
            class="h-6 w-6"
            @click="removeMenuOption(String(digit))"
          >
    ...
    <template v-if="node.type === 'gather'">
      <div class="space-y-1.5">
        ...
        <Input
          type="number"
          :model-value="String(config.max_digits || 10)"
          @update:model-value="
            (v: string) => updateConfig('max_digits', parseInt(v) || 10)
          "
          class="h-8 text-sm"
          min="1"
          max="20"
      ...
      <div class="space-y-1.5">
        ...
        <Input
          ...
          @update:model-value="(v: string) => updateConfig('terminator', v)"
          class="h-8 text-sm"
        />
      </div>
      <div class="space-y-1.5">
    ...
    <template v-if="node.type === 'http_callback'">
      <div class="space-y-1.5">
        ...
        <Input
          ...
          placeholder="https://api.example.com/ivr"
      ...
      <div class="space-y-1.5">
        ...
        <Textarea
          ...
          @update:model-value="(v: string) => updateConfig('body_template', v)"
          placeholder='{"phone": "{{caller_phone}}"}'
          class="min-h-[60px] text-xs font-mono resize-none"
        />
        <p class="text-[10px] text-muted-foreground">
      ...
      <div class="space-y-1.5">
        ...
        <Input
          ...
          @update:model-value="
            (v: string) => updateConfig('response_store_as', v)
          "
    ...
    <template v-if="node.type === 'transfer'">
      <div class="space-y-1.5">
        ...
        <Select
          ...
          @update:model-value="
            (v: any) => updateConfig('team_id', v === 'none' ? '' : v)
          "
        >
      ...
      <div class="space-y-2 mt-3">
        ...
        <div
          v-for="event in callbackEvents"
          :key="event"
          class="border rounded-md"
        >
          <button
            class="w-full flex items-center justify-between px-3 py-1.5 text-xs font-medium hover:bg-muted/50"
            @click="
              updateCallbackField(
                event,
                '_expanded',
                !getCallbackConfig(event)._expanded,
              )
            "
          >
            <span>{{ callbackLabels[event] }}</span>
            <span
              v-if="getCallbackConfig(event).url"
          ...
          >
            <div class="space-y-1 pt-2">
              ...
              <Input
                ...
                @update:model-value="
                  (v: string) => updateCallbackField(event, 'url', v)
                "
                placeholder="https://crm.example.com/api/call"
            ...
            <div class="space-y-1">
              ...
              <Select
                ...
                @update:model-value="
                  (v: any) => updateCallbackField(event, 'method', v)
                "
              >
            ...
            <div class="space-y-1">
              <div class="flex items-center justify-between">
                ...
                <Button
                  ...
                  @click="addCallbackHeader(event)"
                >
                  <Plus class="h-2.5 w-2.5 mr-0.5" /> Add
                </Button>
              </div>
              <div
                v-for="(val, key) in getCallbackConfig(event).headers || {}"
                :key="String(key)"
                class="flex items-center gap-1"
              >
                <Input
                  ...
                  @update:model-value="
                    (v: string) =>
                      updateCallbackHeaderKey(event, String(key), v)
                  "
                  placeholder="Key"
                ...
                <Input
                  ...
                  @update:model-value="
                    (v: string) =>
                      updateCallbackHeaderValue(event, String(key), v)
                  "
                  placeholder="Value"
                  class="h-6 text-[10px] flex-1"
                />
                <Button
                  ...
                  size="icon"
                  class="h-5 w-5"
                  @click="removeCallbackHeader(event, String(key))"
                >
            ...
            >
              ...
              <Textarea
                :model-value="getCallbackConfig(event).body_template || ''"
                @update:model-value="
                  (v: string) => updateCallbackField(event, 'body_template', v)
                "
                :placeholder="`{&quot;phone&quot;: &quot;{{caller_phone}}&quot;, &quot;transfer_id&quot;: &quot;{{transfer_id}}&quot;}`"
                class="min-h-[50px] text-[10px] font-mono resize-none"
              />
              <p class="text-[9px] text-muted-foreground">
                Body text is not a secret vault; header values are encrypted and
                masked.
      ...
      <div class="border rounded-md mt-2">
        <button
          class="w-full flex items-center justify-between px-3 py-1.5 text-xs font-medium hover:bg-muted/50"
          @click="updateConfig('_vars_expanded', !config._vars_expanded)"
        >
        ...
        <div v-if="config._vars_expanded" class="px-3 pb-2 border-t">
          <div class="flex flex-wrap gap-1 pt-2">
            <code
              ...
              :key="v"
              class="bg-muted px-1.5 py-0.5 rounded text-[10px] cursor-pointer hover:bg-muted/80"
              :title="
                v.includes('*') ? 'Available on connect/complete only' : ''
              "
              >{{ v.replace(" *", "").replace(" **", "") }}</code
            >
          </div>
          <p class="text-[9px] text-muted-foreground mt-1.5">
            * on_connect/on_complete only &nbsp; ** on_complete only
          </p>
        </div>
      </div>
    </template>
