...
func productIntegrityStatements() []string {
	statements := []string{
		...
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_provider_integrations_id_org ON provider_integrations(id, organization_id)`,
	...
	foreignKeys := []productTenantForeignKey{
		...
		{"fk_threads_platform_bindings_integration_tenant", "threads_platform_bindings", "integration_id, organization_id", "provider_integrations", "id, organization_id", "RESTRICT"},
		{"fk_threads_platform_bindings_account_tenant", "threads_platform_bindings", "channel_account_id, organization_id", "channel_accounts", "id, organization_id", "RESTRICT"},
