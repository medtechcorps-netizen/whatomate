...
func setupRLSMigrationSchemaTrustTest(t *testing.T, phase string) (*gorm.DB, *gorm.DB, string) {
	t.Helper()
	if phase == "baseline" {
		return setupPreAdditiveRLSMigrationTest(t)
	}
	require.Equal(t, "bridge", phase)
	db, isolatedAdminDB, _, runtimeRole := testutil.OpenIsolatedTestDatabaseOwnedByRole(t)
	require.NoError(t, databasepkg.ApplyTenantRLS(db, runtimeRole))
	dropIdentityReviewOldCoreTriggers(t, db)
	return db, isolatedAdminDB, runtimeRole
...
func TestRLSMigrationSchemaTrustRejectsBeforePreparation(t *testing.T) {
	for _, phase := range []string{"baseline", "bridge"} {
		t.Run(phase, func(t *testing.T) {
			for _, grant := range []struct {
				...
				{name: "public", wantCreate: true, wantError: "pre-existing public.btrim"},
				{name: "direct", wantCreate: true, wantError: "pre-existing public.btrim"},
				{name: "inherited", wantCreate: true, wantError: "public schema CREATE revocation postcondition failed", creatorGroup: true, inherit: true},
				{name: "noinherit_settable", wantError: "can SET ROLE to a public-schema creator", creatorGroup: true},
				{name: "already_revoked", wantError: "pre-existing public.btrim"},
			} {
				t.Run(grant.name, func(t *testing.T) {
					db, isolatedAdminDB, runtimeRole := setupRLSMigrationSchemaTrustTest(t, phase)
					organization := testutil.CreateTestOrganization(t, db)
					account := testutil.CreateTestWhatsAppAccount(t, db, organization.ID)
					const historicalPhoneID = "  schema-trust-phone  "
					require.NoError(t, db.Model(account).Update("phone_id", historicalPhoneID).Error)
					integration := models.ProviderIntegration{
						BaseModel: models.BaseModel{ID: uuid.New()}, OrganizationID: organization.ID,
						Provider: "threads", ManagementMode: "workspace_byo",
						Config:         models.JSONB{"app_id": "schema-trust-threads-app"},
					...
					message := models.Message{
						BaseModel: models.BaseModel{ID: uuid.New()}, OrganizationID: organization.ID,
						WhatsAppAccount: account.Name, ContactID: contact.ID,
						WhatsAppMessageID: "wamid.schema-trust", Direction: models.DirectionIncoming,
					...
					require.NoError(t, db.Raw(`
						...
						WHERE (attribute.attrelid = 'public.provider_integrations'::pg_catalog.regclass
						       AND attribute.attname = 'management_mode')
						   OR (attribute.attrelid = 'public.whatsapp_accounts'::pg_catalog.regclass
						       AND attribute.attname = 'phone_id')
						ORDER BY attribute.attname
					`).Scan(&columnTypes).Error)
					require.Equal(t, []string{"character varying(32)", "character varying(100)"}, columnTypes,
						"the overload regression must retain both real varchar preparation inputs")

					const indexName = "idx_messages_org_inbox_ingested"
					const exactIndexSQL = `CREATE INDEX CONCURRENTLY idx_messages_org_inbox_ingested
						ON public.messages(organization_id, inbox_conversation_id, (COALESCE(ingested_at, created_at)), id)
						WHERE inbox_conversation_id IS NOT NULL AND deleted_at IS NULL`
					require.NoError(t, db.Exec("DROP INDEX CONCURRENTLY public."+indexName).Error)
					interruptedIndex := leaveInterruptedConcurrentIndexBuildTestState(
						t, db, isolatedAdminDB, indexName, exactIndexSQL,
					)
					require.False(t, interruptedIndex.Valid)

					// The invoker-owned functions can be planted while CREATE is available.
					// Their sequence is deliberately owner-only: reaching either body as the
					// migrator would cross the privilege boundary, and nextval survives rollback.
					require.NoError(t, db.Exec("CREATE SEQUENCE public.rereply_prepare_schema_execution_probe").Error)
					require.NoError(t, db.Exec(
						"REVOKE ALL ON SEQUENCE public.rereply_prepare_schema_execution_probe FROM PUBLIC",
					).Error)
					require.NoError(t, db.Exec(
						"REVOKE ALL ON SEQUENCE public.rereply_prepare_schema_execution_probe FROM "+runtimeRole,
					).Error)
					require.NoError(t, db.Exec("GRANT CREATE ON SCHEMA public TO "+runtimeRole).Error)
					require.NoError(t, isolatedAdminDB.Transaction(func(tx *gorm.DB) error {
						if err := tx.Exec("SET LOCAL ROLE " + runtimeRole).Error; err != nil {
							return err
						}
						if err := tx.Exec(`
							CREATE FUNCTION public.btrim(character varying)
							RETURNS text LANGUAGE plpgsql IMMUTABLE SECURITY INVOKER
							SET search_path = pg_catalog, public
							AS $function$
							BEGIN
								PERFORM pg_catalog.nextval('public.rereply_prepare_schema_execution_probe'::pg_catalog.regclass);
								RETURN pg_catalog.btrim($1::text);
							END
							$function$
						`).Error; err != nil {
							return err
						...
						if err := tx.Exec(`
							CREATE FUNCTION public.rereply_prepare_schema_array_append(pg_catalog.oid[], pg_catalog.oid)
							RETURNS pg_catalog.oid[] LANGUAGE plpgsql IMMUTABLE SECURITY INVOKER
							SET search_path = pg_catalog, public
							AS $function$
							BEGIN
								PERFORM pg_catalog.nextval('public.rereply_prepare_schema_execution_probe'::pg_catalog.regclass);
								RETURN pg_catalog.array_append($1, $2);
							END
							$function$
						`).Error; err != nil {
							return err
						}
						// Membership traversal precedes the btrim inventory check. Its
						// array append must not resolve this exact public operator.
						return tx.Exec(`
							CREATE OPERATOR public.|| (
								LEFTARG = pg_catalog.oid[],
								RIGHTARG = pg_catalog.oid,
								FUNCTION = public.rereply_prepare_schema_array_append
							)
						`).Error
					}))
					require.NoError(t, db.Exec("REVOKE CREATE ON SCHEMA public FROM "+runtimeRole).Error)
					for _, signature := range []string{
						"public.btrim(character varying)",
						"public.rereply_prepare_schema_array_append(pg_catalog.oid[],pg_catalog.oid)",
					} {
						var functionState struct {
							Owner                    string `gorm:"column:owner"`
							SecurityDefiner          bool   `gorm:"column:security_definer"`
							RuntimeSequencePrivilege bool   `gorm:"column:runtime_sequence_privilege"`
							OwnerSequencePrivilege   bool   `gorm:"column:owner_sequence_privilege"`
						}
						require.NoError(t, db.Raw(`
							SELECT owner.rolname::text AS owner, function.prosecdef AS security_definer,
								pg_catalog.has_sequence_privilege(CAST(? AS name),
									'public.rereply_prepare_schema_execution_probe', 'USAGE, SELECT, UPDATE') AS runtime_sequence_privilege,
								pg_catalog.has_sequence_privilege(current_user,
									'public.rereply_prepare_schema_execution_probe', 'USAGE, SELECT, UPDATE') AS owner_sequence_privilege
							FROM pg_catalog.pg_proc AS function
							JOIN pg_catalog.pg_roles AS owner ON owner.oid = function.proowner
							WHERE function.oid = CAST(? AS pg_catalog.regprocedure)
						`, runtimeRole, signature).Scan(&functionState).Error)
						require.Equal(t, runtimeRole, functionState.Owner, signature)
						require.False(t, functionState.SecurityDefiner, signature)
						require.False(t, functionState.RuntimeSequencePrivilege, signature)
						require.True(t, functionState.OwnerSequencePrivilege, signature)
					}
					var exactOperatorBinding bool
					require.NoError(t, db.Raw(`
						SELECT EXISTS (
							...
							WHERE namespace.nspname = 'public' AND operator.oprname = '||'
							  AND operator.oprleft = 'pg_catalog.oid[]'::pg_catalog.regtype
							  AND operator.oprright = 'pg_catalog.oid'::pg_catalog.regtype
							  AND operator.oprcode = 'public.rereply_prepare_schema_array_append(pg_catalog.oid[],pg_catalog.oid)'::pg_catalog.regprocedure
							  AND owner.rolname = ?
						)
					`, runtimeRole).Scan(&exactOperatorBinding).Error)
					require.True(t, exactOperatorBinding)

					switch grant.name {
					case "public":
						require.NoError(t, db.Exec("GRANT CREATE ON SCHEMA public TO PUBLIC").Error)
					case "direct":
						require.NoError(t, db.Exec("GRANT CREATE ON SCHEMA public TO "+runtimeRole).Error)
					}
					if grant.creatorGroup {
						groupRole := "rereply_schema_creator_" + strings.ReplaceAll(uuid.NewString(), "-", "")[:16]
						...
						t.Cleanup(func() {
							if err := isolatedAdminDB.Exec("REVOKE CREATE ON SCHEMA public FROM " + groupRole).Error; err != nil {
								t.Errorf("revoke synthetic schema CREATE: %v", err)
							}
							if err := isolatedAdminDB.Exec("REVOKE " + groupRole + " FROM " + runtimeRole).Error; err != nil {
								t.Errorf("revoke synthetic schema-creator membership: %v", err)
							}
							if err := isolatedAdminDB.Exec("DROP ROLE " + groupRole).Error; err != nil {
								t.Errorf("drop synthetic schema-creator role: %v", err)
							}
						})
						if grant.inherit {
							// PG14 uses this role attribute; PG17 copies it into the
							// new membership's inherit_option when GRANT is issued.
							require.NoError(t, isolatedAdminDB.Exec("ALTER ROLE "+runtimeRole+" INHERIT").Error)
						}
						require.NoError(t, isolatedAdminDB.Exec("GRANT "+groupRole+" TO "+runtimeRole).Error)
						require.NoError(t, db.Exec("GRANT CREATE ON SCHEMA public TO "+groupRole).Error)
						var membership struct {
							Inherit         bool `gorm:"column:inherit"`
							Settable        bool `gorm:"column:settable"`
							MigrationMember bool `gorm:"column:migration_member"`
						}
						require.NoError(t, db.Raw(`
							SELECT COALESCE((pg_catalog.to_jsonb(membership)->>'inherit_option')::boolean,
								runtime.rolinherit) AS inherit,
								COALESCE((pg_catalog.to_jsonb(membership)->>'set_option')::boolean, true) AS settable,
								pg_catalog.pg_has_role(current_user, creator.oid, 'MEMBER') AS migration_member
							FROM pg_catalog.pg_auth_members AS membership
							JOIN pg_catalog.pg_roles AS runtime ON runtime.oid = membership.member
							JOIN pg_catalog.pg_roles AS creator ON creator.oid = membership.roleid
							WHERE runtime.rolname = ? AND creator.rolname = ?
						`, runtimeRole, groupRole).Scan(&membership).Error)
						require.Equal(t, grant.inherit, membership.Inherit)
						require.True(t, membership.Settable)
						require.False(t, membership.MigrationMember,
							"creator reachability must be rooted at runtime, not the migration session")
					}
					readSchemaACL := func() string {
						var acl string
						require.NoError(t, db.Raw(`
							SELECT nspacl::text FROM pg_catalog.pg_namespace WHERE nspname = 'public'
						`).Scan(&acl).Error)
						return acl
					}
					beforeACL := readSchemaACL()
					var runtimeCanCreate bool
					require.NoError(t, db.Raw(
						"SELECT pg_catalog.has_schema_privilege(CAST(? AS name), 'public', 'CREATE')", runtimeRole,
					).Scan(&runtimeCanCreate).Error)
					require.Equal(t, grant.wantCreate, runtimeCanCreate)

					readPreparationCatalog := func() ([]string, []string) {
						var fingerprints, relations []string
						require.NoError(t, db.Raw(`
							SELECT function.oid::text || ':' || function.prosrc || ':' || COALESCE(function.proacl::text, '')
							FROM pg_catalog.pg_proc AS function
							JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = function.pronamespace
							WHERE namespace.nspname = 'public'
							  AND function.proname = 'rereply_tenant_policy_additive_fingerprint_v1'
							ORDER BY function.oid
						`).Scan(&fingerprints).Error)
						require.NoError(t, db.Raw(`
							SELECT relation.oid::text || ':' || relation.relname || ':' || COALESCE(relation.relacl::text, '')
							FROM pg_catalog.pg_class AS relation
							JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = relation.relnamespace
							WHERE namespace.nspname = 'public' AND relation.relname IN (
								'whatsapp_coexistence_states', 'whatsapp_identity_review_holds', 'whatsapp_identity_review_members'
							)
							ORDER BY relation.relname
						`).Scan(&relations).Error)
						return fingerprints, relations
					}
					beforeFingerprints, beforeRelations := readPreparationCatalog()
					if phase == "baseline" {
						require.Empty(t, beforeFingerprints)
						require.Empty(t, beforeRelations)
					} else {
						require.Len(t, beforeFingerprints, 1)
						require.Len(t, beforeRelations, 3)
					}
					backfillCalls, verificationCalls := 0, 0
					err := databasepkg.RunRLSMigrationCoordinatorForTest(
						db, &config.DefaultAdminConfig{}, runtimeRole, phase,
						func(*gorm.DB) error { backfillCalls++; return nil },
						func() error { verificationCalls++; return nil },
					)
					require.ErrorIs(t, err, databasepkg.ErrRLSMigrationCatalogQuarantined)
					require.ErrorContains(t, err, grant.wantError)
					require.Zero(t, backfillCalls)
					require.Zero(t, verificationCalls)
					require.Equal(t, beforeACL, readSchemaACL(),
						"schema-trust rejection must roll back its bounded CREATE revocations")
					var sequenceCalled bool
					require.NoError(t, db.Raw(`
						SELECT is_called FROM public.rereply_prepare_schema_execution_probe
					`).Scan(&sequenceCalled).Error)
					require.False(t, sequenceCalled,
						"neither membership traversal nor preparation may execute a planted body, even in a rolled-back transaction")
					afterIndex, exists := readMigrationIndexLifecycleTestState(t, isolatedAdminDB, indexName)
					require.True(t, exists)
					require.Equal(t, interruptedIndex, afterIndex,
						"schema-trust rejection must precede exact interrupted-index reconciliation")
					afterFingerprints, afterRelations := readPreparationCatalog()
					require.Equal(t, beforeFingerprints, afterFingerprints,
						"schema-trust rejection must precede prepare-claim creation or fingerprint publication")
					require.Equal(t, beforeRelations, afterRelations,
						"schema-trust rejection must preserve absent additive relations or the existing legacy catalog exactly")
					var durableIntegration models.ProviderIntegration
					require.NoError(t, db.First(&durableIntegration, "id = ?", integration.ID).Error)
					require.Equal(t, "workspace_byo", durableIntegration.ManagementMode)
					require.Nil(t, durableIntegration.ThreadsAppID, "provider binding preparation must not run")
					var durableAccount models.WhatsAppAccount
					require.NoError(t, db.First(&durableAccount, "id = ?", account.ID).Error)
					require.Equal(t, historicalPhoneID, durableAccount.PhoneID, "phone normalization must not run")
					var durableMessage models.Message
					require.NoError(t, db.First(&durableMessage, "id = ?", message.ID).Error)
					require.Equal(t, message.Metadata, durableMessage.Metadata, "coexistence backfill must not run")
					require.Empty(t, identityReviewOldCoreTriggerBindings(t, db), "activation must remain unpublished")
				})
			}
		})
	}
...
func TestRLSMigrationSchemaTrustRevokesCleanCreateBeforeCallbacks(t *testing.T) {
	for _, phase := range []string{"baseline", "bridge"} {
		t.Run(phase, func(t *testing.T) {
			for _, grantee := range []string{"public", "direct"} {
				t.Run(grantee, func(t *testing.T) {
					db, _, runtimeRole := setupRLSMigrationSchemaTrustTest(t, phase)
					grantRole := runtimeRole
					if grantee == "public" {
						grantRole = "PUBLIC"
					}
					require.NoError(t, db.Exec("GRANT CREATE ON SCHEMA public TO "+grantRole).Error)
					requireCreateRevoked := func(session *gorm.DB) {
						t.Helper()
						var authority struct {
							PublicCreate  bool `gorm:"column:public_create"`
							RuntimeCreate bool `gorm:"column:runtime_create"`
						}
						require.NoError(t, session.Raw(`
							SELECT EXISTS (
							...
							pg_catalog.has_schema_privilege(CAST(? AS name), namespace.oid, 'CREATE') AS runtime_create
							FROM pg_catalog.pg_namespace AS namespace WHERE namespace.nspname = 'public'
						`, runtimeRole).Scan(&authority).Error)
						require.False(t, authority.PublicCreate)
						require.False(t, authority.RuntimeCreate,
							"CREATE must already be revoked before the callback and later ApplyTenantRLS")
					}
					backfillCalls, verificationCalls := 0, 0
					require.NoError(t, databasepkg.RunRLSMigrationCoordinatorForTest(
						db, &config.DefaultAdminConfig{}, runtimeRole, phase,
						func(session *gorm.DB) error {
							backfillCalls++
							requireCreateRevoked(session)
							return databasepkg.VerifyRLSMigrationCallbackScanAuthorityForTest(session, runtimeRole)
						},
						func() error {
							verificationCalls++
							if phase == "bridge" {
							...
							return databasepkg.VerifyPlatformComplianceIdentityReviewCompatibilityForTest(db, runtimeRole)
						},
					))
					require.Equal(t, 1, backfillCalls)
					require.Equal(t, 1, verificationCalls)
					requireCreateRevoked(db)
					if phase == "bridge" {
						require.Equal(t, expectedIdentityReviewOldCoreTriggerBindings(), identityReviewOldCoreTriggerBindings(t, db))
					} else {
						require.Empty(t, identityReviewOldCoreTriggerBindings(t, db))
					}
				})
...
func TestWhatsAppIdentityReviewActivationAllowsLongHistoricalWorkAndCommits(t *testing.T) {
	db := testutil.SetupTestDB(t)
	testutil.TruncateTables(db)
	...
	statement := fmt.Sprintf(`DO $block$ BEGIN
		...
		UPDATE public.contacts SET profile_name = 'cutover-long-running-commit'
		WHERE id = '%s'::uuid;
	...
	assert.GreaterOrEqual(t, time.Since(started), time.Second,
		"historical activation work must be allowed to exceed one second")

	var durable models.Contact
	require.NoError(t, db.Unscoped().First(&durable, "id = ?", contact.ID).Error)
	assert.Equal(t, "cutover-long-running-commit", durable.ProfileName)
}
...
func TestWhatsAppIdentityReviewActivationErrorRollsBackTheWholeStatement(t *testing.T) {
	...
	statement := fmt.Sprintf(`DO $block$ BEGIN
		...
		WHERE id = '%s'::uuid;
		RAISE EXCEPTION 'deliberate activation failure' USING ERRCODE = 'P0001';
	END $block$`, contact.ID)
	...
	assert.Equal(t, originalProfileName, durable.ProfileName,
		"any activation error must roll back every cutover write")
}
