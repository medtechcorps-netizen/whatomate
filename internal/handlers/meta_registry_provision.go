...
func (a *App) provisionMetaRegistryBinding(input metaRegistryProvisionInput) (metaRegistryProvisionResult, error) {
	...
	if input.Name == "" || utf8.RuneCountInString(input.Name) > 100 ||
		...
		input.AccessToken == "" || input.MetaBusinessID == "" || input.AuthorizingMetaUserID == "" ||
	...
	account := models.ChannelAccount{
		...
		ExternalAccountID: input.ExternalAccountID, Status: models.ChannelAccountStatusActive,
		...
		ConnectedAt: &connectedAt, CreatedByID: &input.UserID, UpdatedByID: &input.UserID,
	}
